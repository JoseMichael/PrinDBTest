...
public class MandG
{
	...
	public void MQuery(String tableid, String areacode, int MMPAGES, WriteToDisk obj) 
	{
		wtd = obj;
		...
		//Create lists of datalocation for each answer list, as well as an answer row for final output
		...
		//List of slotted pages, each with a list of strings from that page (phone numbers in this case)
		HashMap<Integer, List<String>> slottedListPhone = new HashMap<>(wtd.getSlottedList(tableid, "Phone"));
		...
		//For each key (bucket) go through the list and separate into pages of size 512
		for(int key:slottedListPhone.keySet())
		{			
			List<String> listOfAllSlottedPageEntries = slottedListPhone.get(key);
			for(int i =0;i<listOfAllSlottedPageEntries.size();i++)
			{
				...
				//if size after adding is less than or eq to 512, add the word to dummypage
				if((totalstringsize + listOfAllSlottedPageEntries.get(i).length()) <= 512)
				{
		...
		for(int x = 0; x <= pageListPhone.size() - 1; x++)
		{
			...
			int index = 0;
			//Check if we have max pages in MM, if we do, get rid of LRU based on TimeStamp
			if(MMPAGELIST.size() == MMPAGES)
			{
				//Find the index of the page to replace
				index = CalcPageToReplace(MMPAGELIST);
				
				String pageswapout = "SWAP OUT T-" + MMPAGELIST.get(index).getData().get(0).getTableName()
						+ " P-" + Integer.toString(getPage(MMPAGELIST.get(index).getData().get(0))) + " B-"
						+ Integer.toString(MMPAGELIST.get(index).getData().get(0).getBucketNo());
				logWriter(pageswapout);
				
				//Overwriting LRU
				MMPAGELIST.put(index, pcm);
				
				String pageswapin = "SWAP IN T-" + MMPAGELIST.get(index).getData().get(0).getTableName()
						+ " P-" + Integer.toString(getPage(MMPAGELIST.get(index).getData().get(0))) + " B-"
						+ Integer.toString(MMPAGELIST.get(index).getData().get(0).getBucketNo());
			...
			else
			{
				...
				String pagecreate = "CREATE T-" + MMPAGELIST.get(index).getData().get(0).getTableName()
						+ " P-" + Integer.toString(getPage(MMPAGELIST.get(index).getData().get(0))) + " B-"
						+ Integer.toString(MMPAGELIST.get(index).getData().get(0).getBucketNo());
				logWriter(pagecreate);
			}
			//Search the page we just put into main memory for our area code and
			List<DataLocation> dummyAnswerList = new ArrayList<DataLocation>(searchPage(MMPAGELIST.get(index).getData(), areacode));
		...
		for (DataLocation dl: answerList)
		{
			...
			{
				List<String> testList = wtd.getSlottedPageForBucketId(dl.getTableName(), "Name", dl.getBucketNo(), dl.getRowNo());
			...
			slottedPage = new ArrayList<>(wtd.getSlottedPageForBucketId(dl.getTableName(), "Name", dl.getBucketNo(), dl.getRowNo()));
			
			//populate repeat test vars
			repeatTest = new ArrayList<>(slottedPage);
			repeatcheckflag = 1;
			
			//calculate pageno using the datalocation in answerlist, but with data size of "name"
			...
			for(int i =0;i<slottedPage.size();i++)
			{
				...
				if((totalstringsize + slottedPage.get(i).length()) <= 512)
				{
					...
					//set row to be the current iterating index + the data size * the pageno we're on
		...
		for(int x = 0; x <= pageListName.size() - 1; x++)
		{
			...
			//Search the page we just put into main memory based on our answer list
			List<DataLocation> dummyAnswerList = new ArrayList<DataLocation>(searchForName(MMPAGELIST.get(index).getData(), answerList));
			for(DataLocation dl: dummyAnswerList)
			{
		...
		for (DataLocation dl: answerList)
		{
			...
			if(repeatcheckflag == 1)
			{
				List<String> testList = wtd.getSlottedPageForBucketId(dl.getTableName(), "Id", dl.getBucketNo(), dl.getRowNo());
				if(repeatTest.equals(testList))
				{
					//THIS IS A REPEAT, DO NOTHING
					continue;
			...
			//gets us the specific slotted page for the bucket we want
			slottedPage = new ArrayList<>(wtd.getSlottedPageForBucketId(dl.getTableName(), "Id", dl.getBucketNo(), dl.getRowNo()));
			repeatTest = new ArrayList<>(slottedPage);
			repeatcheckflag = 1;
		...
		for(int x = 0; x <= pageListId.size() - 1; x++)
		{
			...
			//Search the page we just put into main memory based on our answer list
			List<DataLocation> dummyAnswerList = new ArrayList<DataLocation>(searchForId(MMPAGELIST.get(index).getData(), answerList));
			for(DataLocation dl: dummyAnswerList)
			{
				answerListId.add(dl);
			}
		...
		{
			String output = "MRead: " + answerListId.get(i).getDataString() + "," + answerListName.get(i).getDataString() + "," + answerList.get(i).getDataString();
			logWriter(output);
		...
		{
			String output = "No matches found for area code: " + areacode.toString();
			logWriter(output);
			System.out.println(output);
		}
	}
	...
	public int CalcPageToReplace(Map<Integer, PageColumnMeta> mypages)
	{
		...
		PageColumnMeta LRU = new PageColumnMeta((PageColumnMeta) mypair.getValue());
		int LRUindex = (int)mypair.getKey();
		
		while(i.hasNext())
		{
			mypair = (Map.Entry)i.next();
			PageColumnMeta swap = new PageColumnMeta((PageColumnMeta) mypair.getValue());
			if(LRU.getTimeStamp().compareTo(swap.getTimeStamp()) < 0) //If LRU is less than swap
			{
	...
	public List<DataLocation> searchPage(List<DataLocation> datastrings, String phonequery)
	{
		//bucket, offset
		...
		{
			//if the string contains the area code, add its location to index array
	...
	public List<DataLocation> searchForName(List<DataLocation> nameLocs, List<DataLocation> answerLocs)
	{
		...
		for(DataLocation names: nameLocs)
		{
			...
			{
				if(names.getBucketNo() == answers.getBucketNo() && names.getRowNo() == answers.getRowNo())
				{
					DataLocation dummyData = new DataLocation();
	...
	public List<DataLocation> searchForId(List<DataLocation> idLocs, List<DataLocation> answerLocs)
	{
		...
		for(DataLocation ids: idLocs)
		{
			for(DataLocation answers: answerLocs)
			{
				if(ids.getBucketNo() == answers.getBucketNo() && ids.getRowNo() == answers.getRowNo())
				{
					DataLocation dummyData = new DataLocation();
					dummyData.setBucketNo(answers.getBucketNo());
